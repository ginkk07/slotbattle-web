...
export function BattleView({ state, catalog, onCommand, onComplete, completionLabel = "再戰一場", error }: { state: Battle; catalog: Catalog; onCommand: (command: Command) => string | undefined; onComplete: () => void; completionLabel?: string; error?: string }) {
  ...
  function selectSkill(id: string | null) { setPanel("action"); setSkillId(id); setSelected([]); }

  return <div className="battle-screen" aria-busy={busy}>
    <Arena state={scene} catalog={catalog} hit={hit} group={group} busy={busy} />
    <section className="player-ui-panel" aria-label="玩家操作介面">
    ...
    {state.phase === "cards" && !retain && !state.outcome && <div className="command-row">
      <button type="button" className={`item-button ${panel === "bag" ? "active" : ""}`} aria-label="物品" aria-pressed={panel === "bag"} onClick={() => setPanel(panel === "bag" ? "action" : "bag")}><BattleIcon name="bag" /></button>
      <div className="skill-tray" aria-label="選擇技能">{player.skills.map((owned) => {
    ...
    {(state.phase !== "cards" || retain || !!state.outcome) && <nav className="battle-nav" aria-label="戰鬥操作面板">
    ...
    <div className="battle-controls">
      ...
      <section className="decision-panel">
        ...
        {retain && <div className="hand-heading"><h2>保留卡片</h2><span>{selected.length} / {Math.floor(state.ap / catalog.rules.retainCost)} 張</span><button type="button" className="quiet" disabled={locked || immobilized} onClick={() => selected.length ? setSelected([]) : setSelected(state.hand.filter((card) => !isCursed(card) && (card.expiresAt === undefined || card.expiresAt > state.round)).map((card) => card.id))}>{selected.length ? "取消全選" : "全選"}</button></div>}
