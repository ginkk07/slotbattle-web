...
export function BattleView({ state, catalog, onCommand, onComplete, completionLabel = "再戰一場", error }: { state: Battle; catalog: Catalog; onCommand: (command: Command) => string | undefined; onComplete: () => void; completionLabel?: string; error?: string }) {
  ...
  return <div className="battle-screen" aria-busy={busy}>
    ...
    <div className="player-resource-bar">
      <div className="resource-hp"><span>生命</span><strong>{visiblePlayer.hp}<small> / {visiblePlayer.maxHp}</small></strong><div className="hp-track"><span style={{ width: `${100 * visiblePlayer.hp / visiblePlayer.maxHp}%` }} /></div></div>
      <div><span>🛡️ 護甲</span><strong key={visiblePlayer.armor} className="number-change">{visiblePlayer.armor}</strong></div>
      <div className="pool-resource"><span>⚔️ 戰鬥傷害</span><strong key={visiblePlayer.pool} className="number-change">{visiblePlayer.pool}</strong></div>
      <div className="ap-resource"><span>❇️ AP</span><strong>{scene.ap}<small> / {scene.apCapacity}</small></strong></div>
    </div>
    <nav className="battle-nav" aria-label="戰鬥操作面板">
      <button type="button" className={panel === "action" ? "active" : ""} aria-pressed={panel === "action"} onClick={() => setPanel("action")}>{state.outcome ? "戰鬥結果" : state.phase === "slots" ? "拉霸" : retain ? "保留卡片" : "手牌與技能"}</button>
      <button type="button" className={panel === "bag" ? "active" : ""} aria-pressed={panel === "bag"} onClick={() => setPanel("bag")}>背包</button>
      <button type="button" className={panel === "log" ? "active" : ""} aria-pressed={panel === "log"} onClick={() => setPanel("log")}>紀錄</button>
    </nav>
    <div className="battle-controls">
      {busy ? <section className="battle-result"><span className="result-sigil">⚔️</span><p>回合 {scene.round}</p><h2>{state.round !== scene.round || state.outcome ? "回合結算" : "效果結算"}</h2><p>{hit?.label}</p></section> : state.outcome && panel === "action" ? <section className="battle-result" role="status"><span className="result-sigil">{state.outcome === "victory" ? "✧" : "◇"}</span><p>戰鬥結束</p><h2>{state.outcome === "victory" ? "勝利" : state.outcome === "draw" ? "雙方全滅" : "冒險暫告一段落"}</h2><button type="button" className="primary" onClick={onComplete}>{completionLabel}</button></section> :
      ...
      state.phase === "slots" ? <section className="slot-entry"><div className="mini-board" aria-hidden="true">{state.board.map((symbol, index) => <span key={index}>{catalog.rules.symbols[symbol].icon}</span>)}</div><p className="eyebrow">拉霸階段</p><h2>準備這回合的卡片</h2><p>查看敵人的行動，決定要不要重轉。</p><button type="button" className="primary" onClick={() => setSlotsOpen(true)}>打開拉霸 <span>→</span></button><small>剩餘 {state.ap} AP · 確認盤面後進入出牌階段</small></section> :
      <section className="decision-panel">
        {!retain && <div className="skill-tray" aria-label="選擇技能">{player.skills.map((owned) => {
          const definition = catalog.skills[owned.id]; const current = definition.levels[owned.level - 1]; const sealed = state.sealedSkills.includes(owned.id);
          return <button type="button" className={`skill-tile rarity-${definition.rarity} ${skillId === owned.id ? "active" : ""}`} aria-pressed={skillId === owned.id} disabled={locked || state.phase !== "cards" || immobilized || sealed} key={owned.id} onClick={() => selectSkill(skillId === owned.id ? null : owned.id)}><span><strong>{definition.name}</strong><small>Lv.{owned.level}{sealed ? " 封印" : ""}</small></span><b>{current.condition}</b></button>;
        })}{!player.skills.length && <span className="muted">尚未持有技能</span>}</div>}
        {level && !retain && <div className="material-hint"><div><strong>{skill?.name}</strong><button type="button" className="quiet" onClick={() => selectSkill(null)}>取消</button></div><p>{level.description}</p><span className={canUseSkill ? "condition-met" : "muted"}>{canUseSkill ? "✓ 條件達成" : `選擇卡片 · ${level.condition}`}</span></div>}
        ...
        <div className="hand-heading"><h2>{retain ? "保留卡片" : "手牌"} <small>{state.hand.length}</small></h2><span>{retain ? `${selected.length} / ${Math.floor(state.ap / catalog.rules.retainCost)} 張` : `已選 ${selected.length} 張`}</span><button type="button" className="quiet" disabled={locked || (immobilized && !retain)} onClick={() => selected.length ? setSelected([]) : setSelected(state.hand.filter((card) => !isCursed(card) && (!retain || card.expiresAt === undefined || card.expiresAt > state.round)).map((card) => card.id))}>{selected.length ? "取消全選" : "全選"}</button></div>
        {state.hand.length ? <Hand cards={state.hand} selected={selected} catalog={catalog} onToggle={toggleCard} disabled={locked || (immobilized && !retain)} material={!!skill && !retain} skullBoost={skullBoost} retainingRound={retain ? state.round : undefined} /> : <div className="empty-hand"><span>◇</span><p>手牌已用完</p><small>結束回合，發動戰鬥攻擊。</small></div>}
      </section>}
    </div>
    {!state.outcome && panel === "action" && state.phase !== "slots" && <div className="action-dock">
      {busy ? <p className="resolving-label" role="status">戰鬥結算中…</p> : retain ? <><button type="button" onClick={() => command({ type: "cancelEnd" })}>返回</button><button type="button" className="primary" disabled={selected.length * catalog.rules.retainCost > state.ap} onClick={() => command({ type: "end", cardIds: selected })}>{selected.length ? `保留 ${selected.length} 張並攻擊` : "不保留，發動攻擊"}</button></> : <><div className="selection-preview">{skill ? <span>{canUseSkill ? "條件達成" : "選擇技能卡片"}</span> : <><span>卡片攻防</span><b>⚔️ +{preview.attack} <em>🛡️ +{preview.armor}</em></b></>}</div><div className="main-actions"><button type="button" className="end-turn" onClick={() => command({ type: "beginEnd" })}>結束回合</button><button type="button" className="primary" disabled={immobilized || (skill ? !canUseSkill : !selected.length)} onClick={() => command(skill ? { type: "skill", skillId: skill.id, cardIds: selected } : { type: "play", cardIds: selected })}>{skill ? "使用技能" : `出牌${selected.length ? ` · ${selected.length}` : ""}`}</button></div></>}
    </div>}
    ...
    <Modal open={slotsOpen && state.phase === "slots" && !locked} title="命運轉輪" eyebrow={`回合 ${String(state.round).padStart(2, "0")} · 拉霸階段`} onClose={() => setSlotsOpen(false)} className="reels-modal" error={error}>
      <div className="modal-resource"><span>行動點數</span><strong>❇️ {state.ap}<small> / {state.apCapacity} AP</small></strong></div>
