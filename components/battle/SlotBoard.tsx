...
export function SlotBoard({ state, catalog, onCommand, disabled = false }: { state: Battle; catalog: Catalog; onCommand: (command: Command) => void; disabled?: boolean }) {
  ...
  return <section className="board-panel" aria-label="拉霸盤面">
    ...
    <div className="reel-machine">
      ...
      <div className="slot-grid">{state.board.map((symbol, index) => <div key={index} className={`slot-cell symbol-${symbol} ${state.locked.includes(index % rules.columns) ? "locked" : spinning ? "spinning" : ""}`} title={rules.symbols[symbol].label} style={{ "--reel-delay": `${(index % rules.columns) * 45}ms` } as CSSProperties}><span><img src={cardArt.symbols[symbol]} alt={rules.symbols[symbol].label} draggable={false} /></span></div>)}</div>
    </div>
